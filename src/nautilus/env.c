...
char** nk_get_environ(struct nk_env* env){
    uint64_t i;
    ENV_LOCK_CONF;
    //char ** _environ;
    ENV_LOCK(env);
    ...
    _environ = malloc(sizeof(char*)*(env->num_keys));
    
    for (i=0;i<env->num_keys;i++) {
	if (KEY(env,i)) {
	   _environ[i] = KEY(env,i);
	}
    }
...
#define KMP_ENV "KMP"
static int handle_setenv (char * buf, void * priv)
{
   
  char key[256], value[256];
  if (sscanf(buf,"setenv %s %s",key,value)==2) {
        struct nk_env *env = nk_env_find(KMP_ENV);
        if(!env){
          nk_vc_printf("KMP env not found\n");
	  return 0;
	}
	if(nk_env_update(env,key,value)==-1){
	   nk_env_insert(env, key, value);
	}
	//nk_env_dump_env(env);
	return 0;
    } else {
	nk_vc_printf("setenv key=name\n");
	return 0;
    }
}


static struct shell_cmd_impl set_env_impl = {
    .cmd      = "setenv",
    .help_str = "setenv key=value",
    .handler  = handle_setenv,
};
nk_register_shell_cmd(set_env_impl);
